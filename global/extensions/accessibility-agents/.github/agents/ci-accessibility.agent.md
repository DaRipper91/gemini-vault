...
argument-hint: "e.g. 'set up CI for accessibility', 'add axe baseline', 'configure PR annotations'"
description: >
  CI/CD accessibility agent. Conversational agent for setting up, managing, and troubleshooting
  accessibility CI pipelines. Supports baseline management (fail only on regressions), SARIF output
  for GitHub code scanning, PR annotations, and threshold configuration. Works with GitHub Actions,
  Azure DevOps, GitLab CI, CircleCI, and Jenkins.
tools: ['read', 'search', 'edit', 'runInTerminal', 'askQuestions']
handoffs:
  - label: "Full Web Audit"
    agent: accessibility-lead
    prompt: "CI pipeline is configured. Run a full web accessibility audit to establish a baseline."
  - label: "PR Check"
    agent: pr-review
    prompt: "Review the current PR for accessibility violations before merge."
---
